...
describe("NodeExternalResourceService", () => {
  ...
  describe("#scrapeWebpage()", () => {
    describe("when not cached", () => {
      ...
      it("fetches an html from the server at the given url and extract essential web page information", async () => {
        server.use(
        ...
        expect(
          await new NodeExternalResourceService().scrapeWebpage(
      ...
      it("throws an error if the server responded with non-2xx code (302)", async () => {
        ...
        await expect(() =>
          new NodeExternalResourceService().scrapeWebpage(
            "https://dummy.kohei.dev/"
          )
        ).rejects.toThrow();
      });
    ...
    describe("when cached", () => {
      beforeEach(() => {
        mockRedisInstance.hgetall.mockReset().mockResolvedValue({
          href: "https://dummy.kohei.dev/?hl=en-US",
          title:
            "Leo a diam sollicitudin tempor id. Sit amet volutpat consequat mauris nunc congue nisi vitae",
          description:
        ...
        mockRedisInstance.hset.mockReset().mockResolvedValue(undefined);
      });

      it("returns data without fetching the actual HTML", async () => {
        expect(
          await new NodeExternalResourceService().scrapeWebpage(
            "https://dummy.kohei.dev/"
          )
