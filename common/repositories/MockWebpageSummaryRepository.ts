...
export default class MockWebpageSummaryRepository implements WebpageSummaryRepository {
  getByURL(url: URL): Promise<WebpageSummary> {
    ...
    if (`${url}` === "https://localhost:0/loading-forever") {
      return new Promise(() => {});
    }

    if (`${url}` === "https://localhost:0/no-image") {
      return Promise.resolve({
        url: new URL("https://www.kohei.dev/posts/react-content-loader?hl=en-US"),
        title: "Implementing Loading Placeholder with React Content Loader",
        description: "I spent a year-end vacation to implement loading placeholders on this website. I’ll introduce it.",
    ...
    if (`${url}` === "https://localhost:0/unavailable") {
